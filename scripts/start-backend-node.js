...
const serverDir = path.join(__dirname, '..', 'apps', 'backend-node-compat');

// 检查server目录是否存在
if (!fs.existsSync(serverDir)) {
    console.error('❌ apps/backend-node-compat 目录不存在，请确保项目结构正确');
    process.exit(1);
}
...
if (!fs.existsSync(packageJsonPath)) {
    console.error('❌ apps/backend-node-compat/package.json 不存在');
    process.exit(1);
}
