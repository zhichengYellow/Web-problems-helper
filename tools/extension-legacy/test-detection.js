        ...
        },
        {
            ...
            html: `
                <div class="exam-question">
                    <div class="question-text">3. JavaScript和Java是同一个公司开发的。</div>
                    <div class="true-false-options">
                        <label><input type="radio" name="q3" value="true"> 正确</label>
                        <label><input type="radio" name="q3" value="false"> 错误</label>
                    </div>
                </div>
            `,
            expected: {
                title: 'JavaScript和Java是同一个公司开发的。',
                type: 'true_false',
                options: 2
            }
        },
...
// 导出测试函数供外部调用
window.runDetectionTests = function() {
    const script = document.createElement('script');
    script.textContent = `(${function() {
        console.log('🚀 开始题目检测测试...');
        ${runTests.toString().replace('async function runTests()', 'async function runTests()')}
        runTests();
    }})();`;
    document.head.appendChild(script);
};

console.log('📝 检测测试脚本加载完成。运行 window.runDetectionTests() 开始测试。');