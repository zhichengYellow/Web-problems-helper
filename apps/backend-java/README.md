...
这是一个与插件调用契约等价的 Java 实现（历史上存在 Node 后端 `apps/backend-node-compat/`，现已收敛为兼容转发代理），用于：
...
插件侧默认调用：`http://localhost:3001/api/chat`（legacy 扩展见 `apps/extension-chrome-legacy/config.js` 和 `apps/extension-chrome-legacy/hunyuan-service.js`）。
...
- apps/backend-java/SPRING-AI-GUIDE.md
